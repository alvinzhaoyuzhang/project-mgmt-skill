...
> ⚠️ **重要 · wizard 文案必须 verbatim 展示给用户,严禁压缩/简化/重新组织**
>
> 进入 wizard 任何阶段(D/E/F/G/J)时,你必须从 `wizard-screens.md` 找到对应"屏 X"
> 章节,**把代码块原样复制给用户**(包括 ⓘ 注释、`[1] [2] [3]` 编号格式、emoji、缩进)。
>
> 不要"提炼成 3 行"。不要"做一个简洁版"。不要把 `[1]` 改成 `•`。
>
> 这些文案里嵌入了**消歧义提示和安全网触发条件**(详见 wizard-screens.md 顶部"对话节奏"
> 章节),压缩文案 = 跳过这些防护 = 用户被推到错误架构 = 整个 skill 价值归零。
>
> 这是**流程契约**,不是建议。
